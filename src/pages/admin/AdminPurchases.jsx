...
const COST_FIELDS = [
  ...
  { key: 'dealer_fee',         label: 'Dealer Fee' },
...
function blankForm(detail, purchase) {
  return {
    ...
    bid_price:           Math.round(Number(detail?.bid_price)           || 0),
    auction_charges:     Math.round(Number(detail?.auction_charges)     || 0),
    dealer_fee:          Math.round(Number(detail?.dealer_fee)          || 0),
    nipponbid_commission:Math.round(Number(detail?.nipponbid_commission)|| 0),
    is_third_party:      !!detail?.is_third_party,
    third_party_fee:     Math.round(Number(detail?.third_party_fee)     || 0),
    transportation:      Math.round(Number(detail?.transportation)      || 0),
    loading_custom:      Math.round(Number(detail?.loading_custom)      || 0),
    others_commission:   Math.round(Number(detail?.others_commission)   || 0),
    tax_10_percent:      Math.round(Number(detail?.tax_10_percent)      || 0),
    radiation_photos:    Math.round(Number(detail?.radiation_photos)    || 0),
    custom_fee:          Math.round(Number(detail?.custom_fee)          || 0),
    freight:             Math.round(Number(detail?.freight)             || 0),
    recycle:             Math.round(Number(detail?.recycle)             || 0),
    others:              Math.round(Number(detail?.others)              || 0),
  }
}
